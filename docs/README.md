...
Keyzori is a self-hosted license server for software products. Instance operators manage it through the authenticated admin API or local CLI; licensed product users receive no operator access.
...
| Configure server and Stripe | [Configuration](configuration.md) |
...
Start with the repository [quick start](../README.md#quick-start), then use `keyzori admin` or the authenticated `/admin/*` API to create the first customer and license.
